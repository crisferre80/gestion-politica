...
-- Archivo: project/sql/create_schema_with_rls.sql
...
-- 3) Tabla collection_points (puntos / centros / refererencias por barrio)
...
-- 4) Tabla collection_claims (reclamos por recicladores/dirigentes para un punto)
...
-- collection_points: lectura pública limitada, propietarios y admins pueden CRUD
...
DROP POLICY IF EXISTS "collection_points_select_public_or_owner_or_admin" ON collection_points;
CREATE POLICY "collection_points_select_public_or_owner_or_admin" ON collection_points
  FOR SELECT USING (
    status = 'available'
...
DROP POLICY IF EXISTS "collection_points_insert_owner" ON collection_points;
CREATE POLICY "collection_points_insert_owner" ON collection_points
  FOR INSERT WITH CHECK (
    user_id = auth.uid()::uuid
    OR (EXISTS (SELECT 1 FROM profiles p2 WHERE p2.user_id = auth.uid()::uuid AND p2.is_admin = true))
  );
...
DROP POLICY IF EXISTS "collection_points_update_owner_or_admin" ON collection_points;
CREATE POLICY "collection_points_update_owner_or_admin" ON collection_points
  FOR UPDATE USING (
    user_id = auth.uid()::uuid
    OR (EXISTS (SELECT 1 FROM profiles p2 WHERE p2.user_id = auth.uid()::uuid AND p2.is_admin = true))
  ) WITH CHECK (
...
DROP POLICY IF EXISTS "collection_points_delete_owner_or_admin" ON collection_points;
CREATE POLICY "collection_points_delete_owner_or_admin" ON collection_points
  FOR DELETE USING (
    user_id = auth.uid()::uuid
    OR (EXISTS (SELECT 1 FROM profiles p2 WHERE p2.user_id = auth.uid()::uuid AND p2.is_admin = true))
  );

-- collection_claims: crear reclamo debe ser realizado por reciclador (recycler_id == auth.uid())
ALTER TABLE collection_claims ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "collection_claims_insert_recycler" ON collection_claims;
CREATE POLICY "collection_claims_insert_recycler" ON collection_claims
  FOR INSERT WITH CHECK (
...
DROP POLICY IF EXISTS "collection_claims_select_related" ON collection_claims;
CREATE POLICY "collection_claims_select_related" ON collection_claims
  FOR SELECT USING (
    recycler_id = auth.uid()::uuid
    OR user_id = auth.uid()::uuid
    OR (EXISTS (SELECT 1 FROM profiles p2 WHERE p2.user_id = auth.uid()::uuid AND p2.is_admin = true))
...
DROP POLICY IF EXISTS "collection_claims_update_recycler_or_admin" ON collection_claims;
CREATE POLICY "collection_claims_update_recycler_or_admin" ON collection_claims
  FOR UPDATE USING (
    recycler_id = auth.uid()::uuid
    OR (EXISTS (SELECT 1 FROM profiles p2 WHERE p2.user_id = auth.uid()::uuid AND p2.is_admin = true))
  ) WITH CHECK (
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'collection_points_updated_at_trg') THEN
    CREATE TRIGGER collection_points_updated_at_trg
    BEFORE UPDATE ON collection_points
    FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'collection_claims_updated_at_trg') THEN
    CREATE TRIGGER collection_claims_updated_at_trg
...
BEGIN
  ...
  -- collection_points: claim_id, recycler_id, pickup_time, photo_url, completed_at
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_points' AND column_name='claim_id') THEN
    ALTER TABLE collection_points ADD COLUMN claim_id uuid NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_points' AND column_name='recycler_id') THEN
    ALTER TABLE collection_points ADD COLUMN recycler_id uuid NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_points' AND column_name='pickup_time') THEN
    ALTER TABLE collection_points ADD COLUMN pickup_time timestamptz NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_points' AND column_name='photo_url') THEN
    ALTER TABLE collection_points ADD COLUMN photo_url text NULL;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_points' AND column_name='completed_at') THEN
    ALTER TABLE collection_points ADD COLUMN completed_at timestamptz NULL;
  END IF;

  -- collection_claims: renombrar point_id a collection_point_id si existe
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_claims' AND column_name='point_id') AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='collection_claims' AND column_name='collection_point_id') THEN
    ALTER TABLE collection_claims RENAME COLUMN point_id TO collection_point_id;
  END IF;
