...
function PuntosRecoleccion() {
  ...
  const fetchPoints = React.useCallback(async () => {
    try {
      let query = supabase
        ...
        .select(`
          *,
          profiles!concentration_points_user_id_fkey(
            name,
            email,
            phone
          )
  ...
  const handleDeletePoint = async (pointId: string) => {
    if (!user) {
      setError('Debes iniciar sesión para eliminar Centros de Movilizaciòn');
      return;
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Botón volver al Panel del reciclador (ahora dentro y arriba del título, con margen inferior en mobile) */}
          ...
          <button
            ...
            className="bg-blue-100 hover:bg-blue-200 text-blue-800 px-4 py-2 rounded-md flex items-center gap-2 shadow-lg transition-colors border border-blue-200 md:static md:mt-0 mt-2"
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-blue-700 animate-bounce drop-shadow-lg tracking-wide bg-white/70 px-4 py-2 rounded-lg shadow-md">
              Centros de Movilizaciòn
            </h1>
            <p className="text-gray-700 mt-2 bg-white/60 rounded px-2 py-1 shadow-sm">
              Encuentra Centros de Movilizaciòn cercanos o registra el tuyo
            </p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6 border border-blue-100">
          ...
          <p className="text-gray-700 mb-2">
            Aquí puedes ver todos los Centros de Movilizaciòn creados por los Dirigentes de la provincia. Haz zoom o mueve el mapa para explorar los diferentes puntos disponibles.
          </p>
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-4">Centros de Movilizaciòn Disponibles</h2>
        {loading ? (
          <div className="bg-white rounded-lg shadow-sm p-8 text-center">
            <p className="text-gray-500 mb-4">Cargando Centros de Movilizaciòn...</p>
          </div>
